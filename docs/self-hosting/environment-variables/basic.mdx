...
- Type: Optional
- Description: HTTP endpoint for the governed Alexandria bridge used by Bulma to retrieve approved Totum artifacts and context packs.
- Default: `-`
- Example: `https://cgpkfhrqprqptvehatad.supabase.co/functions/v1/alexandria-mcp`

### `ALEXANDRIA_MCP_TOKEN`

- Type: Optional
- Description: Shared token sent in the `x-alexandria-token` header when calling the Alexandria bridge. When both variables are set, the default Bulma inbox agent enables the Alexandria tool automatically.
- Default: `-`
- Example: `totum-issued-bridge-token`
